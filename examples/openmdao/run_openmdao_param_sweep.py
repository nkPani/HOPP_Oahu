...
if __name__ == "__main__":
    ### build the model
    prob = om.Problem()
    prob.model.add_subsystem('hybrid_system', HybridSystem(location=flatirons_site, battery=False, grid=True, sim_duration_years=25, hybrid_rated_power=250), promotes=['*'])

    ### setup the optimization
    prob.driver = om.ScipyOptimizeDriver()
    # prob.driver.options['optimizer'] = 'SLSQP'
    prob.driver = om.DOEDriver(om.FullFactorialGenerator(levels=108))
    ...
    # prob.model.add_objective('hybrid_npv', ref=-1.e8)
    # prob.model.add_objective('hybrid_lcoe_real', ref=1.)  
    # prob.model.add_objective('hybrid_annual_energy', ref=-1.e8)
