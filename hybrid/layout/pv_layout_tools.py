...
def find_best_gcr(
        ...
        min_gcr: float = 0.0,
        max_gcr: float = 1.0,
        ) -> Tuple[float, int, Tuple[int, float, Polygon]]:
    """
    ...
    """
    best: Tuple[float, int, Tuple[int, float, Polygon]] = (0.0, 0, [])
...
def find_best_solar_size(
        ...
        min_size,
        max_size
        ) -> Tuple[float, int, Tuple[int, float, Polygon], np.ndarray]:
    """
    Finds the smallest size that fits max_num_modules. If that isn't possible, it fits as many modules as it can.
    """
    best: Tuple[float, int, Tuple[int, float, Polygon], np.ndarray] = (0.0, 0, [], Point(0, 0).buffer(.01), np.zeros(2))
    prepared_site = prep(site_shape)
...
def place_solar_strands(max_num_modules: int,
                        ...
                        ) -> Tuple[int, Tuple[int, float, LineString]]:
    """
    ...
    strands: Tuple[int, float, Polygon] = []
...
def get_flicker_loss_multiplier(flicker_data: Tuple[float, np.ndarray, np.ndarray, np.ndarray],
                                ...
                                primary_strands: Tuple[int, float, Polygon],
